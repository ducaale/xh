...
impl ClonedRequest {
    fn inner(self) -> Request {
        match self {
            ClonedRequest::Full(request) | ClonedRequest::Partial(request) => request,
        }
    }
}
...
fn get_next_request(request: ClonedRequest, response: &Response) -> Option<Request> {
    ...
    T: FnMut(Response, &mut Request) -> Result<()>,
{
    ...
    pub fn execute(&mut self, first_request: Request) -> Result<Response> {
        let mut request = clone_request(&first_request);
        ...
        while let Some(mut next_request) = get_next_request(request, &response) {
            ...
            request = clone_request(&next_request);
